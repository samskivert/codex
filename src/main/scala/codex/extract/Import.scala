...
object Import {
  ...
  /** Extracts the fully qualified name for an import and then determines exactly where and how it
    ...
    */
  def computeInfo (refFile :File, fqName :String) :String = {
    var pkgLine = -1
    ...
    val lang = refFile.getName.substring(refFile.getName.lastIndexOf(".")+1)
    val kinds = kindsByLang.getOrElse(lang, Set[String]())
    ...
    val newimp = Import(false, fqName)
    // if we have no import groups, insert this import after the package statement
    if (igroups.isEmpty) "match " + fqName + " " + (pkgLine+1) + " preblank"
    else {
      ...
      // is zero); this probably means the file jams all imports together in one big group
        ...
        igroups find(_.imports.head.compareTo(newimp) > 0) match {
          ...
          case Some(ig) => "match " + fqName + " " + ig.lineno + " postblank"
          ...
          case None => "match " + fqName + " " + igroups.last.linenoAfter + " preblank"
        }
      } else {
        ...
        "match " + fqName + " " + best.linenoFor(newimp) + " noblank"
      }
    }
  }
  ...
  private case class Import (static :Boolean, fqName :String) {
    ...
    private def collate (fqName :String) =
      if (fqName.startsWith("java.")) -2
